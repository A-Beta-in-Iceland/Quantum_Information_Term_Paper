...
Similarly, with alteration of the size of qubit chain size, entanglement methods, and other details, both an teleportation and unitary transformation can be done over this qubit chain with specified measurement pattern. The paper will be organized as follows: in Sec.\ref{cluster state}, I will present how to prepare a chain of qubits, known as a cluster state, where the computation happens upon. In Sec.\ref{qm gates}, I will give the example of construction CNOT gates and $SU(2)$ rotations using this scheme. Finally, in Sec.\ref{concatenation}, I will show how a general quantum circuit can be realized as an overall measurement pattern by concatenating single quantum gate clusters.\par
...
\section{Construction of Single Quanutm State}\label{qm gates}
\subsection{CNOT}
\begin{figure}[h!]
    \centering
...
In this paper, I have summarized the measurement based quantum computation scheme. Motivated from quantum teleportation, measurement on a string of qubits can both propagate and perform unitary transformation on input information. This require proper entanglement of the qubits to form a cluster state using some unitary transformation described in Sec.\ref{cluster state}. Then, with special measurment pattern, whether measurement must be performed in order or simultaneously, quantum gates can be realized on this cluster, with a byproduct that can be corrected based on the measurement result. Finally, concatenation of a network of quantum gates is simply overlapping the cluster for individual quantum gate, which has been justified in Sec.\ref{concatenation}.\par

%% Comparison on efficiency of measurement based quantum computing comparing to quantum circuit construction

In order to see the measurement based quantum computation provides for an universal model of quantum computing, its resource consumption must be reasonable. Hence, an analysis on temporal, operational, and spacial resources as well as the comparism to their quantum circuit construction conterparts are required. 

Starting from the temporal resources, the temporal resource consumption is measured by finding the minimum measurement rounds to which the measurement can be compactified. For a CNOT gate on mesurement based quantum computation, only one measurement is needed. For a general rotation, three measurements are needed. Combing those two aspects, the following inequality is established
\begin{align*}
    T \le 3T_{qc}
\end{align*}\par

Then, we can further analyize for spacial and operational resource consumption. Consider a general cluster of height $h$ and width $w$, so that the spacial resource consumption is $S = w \times h$. As can be observed similar in CNOT gate case, the height is to construct different wires, hence the height is $h = 4 S_{gc}-3$. Further, the vertically on at least one of the wire, there must be one quantum gate, hence the width is $w = 6  S_{qc}T_{qc} + 1$. Since each measurement can contain at most one quantum gate,
\begin{align*}
...
All temporal, operational, and spacial resources are polynomial as compare to the quantum circuit construction. Hence, measurement based quantum computation has a similar efficiency as the quantum circuit model and is indeed an efficient computation scheme.
